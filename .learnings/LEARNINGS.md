...
6. **可视化优先**：每个核心概念必须配图示，防止视觉疲劳
7. **问题即记录**：遇到问题立即记录到 ERRORS.md，解决方案同步更新
8. **定期精简**：每周整理文档，移除冗余，保持精简高效
9. **主动对照检查**：每次修改后必须对照标准模板检查，发现缺失立即补充
...
- **可视化图示（强制）**：
  - 每个核心概念必须配图示（决策树/流程图/对比卡片/关系图）
  - 图示位置：紧跟 concept-box，在费曼类比之前
  - 目的：防止大量文字阅读导致的视觉疲劳
  - 工具：SVG inline 或 styled div（避免外部图片）
- 费曼类比：每个核心概念一个，中文，生活化
- 真实案例：知名公司/历史事件
- Pitfall 汇总表：至少 5 个陷阱

**可视化类型选择：**
- **决策树**：用于判断流程（如"这个成本要不要算进 NPV？"）
- **流程图**：用于步骤流程（如"Depreciation Tax Shield 如何运作"）
- **对比卡片**：用于概念对比（如"Real vs Nominal"）
- **关系图**：用于概念关系（如"Ch.5 完整概念关系图"）
...
- [ ] **Lecture 有可视化图示（每个核心概念，紧跟 concept-box）**
...
**目标：** 无论是否切换新窗口，都能用最佳方案、最快效率、最好效果做出课件

### 定期精简（每周执行）
**每周五晚上或周末，执行文档精简：**

1. **检查冗余内容**
   - 阅读 LEARNINGS.md、ERRORS.md、AGENTS.md
   - 识别重复、过时、冗长的内容
   - 标记需要精简的部分
...
4. **备份原始版本**
   - 精简前创建备份：`LEARNINGS.md.backup-YYYY-MM-DD`
   - 保留最近 3 个版本的备份
   - 超过 3 个月的备份可删除

**目标：** 保持文档精简高效，减少阅读时间，提高工作效率

### 每次制作课件后
1. 记录到 `memory/YYYY-MM-DD.md`
2. 如有新问题，立即更新 `.learnings/ERRORS.md`
3. 如有新经验，立即更新 `.learnings/LEARNINGS.md`
4. 如有流程优化，立即更新 `AGENTS.md`
...
*经验沉淀文档 v2.0 - 2026-03-18 更新*
*适用于所有课程的通用课件制作方法论*
*增加：可视化要求、问题即记录、定期精简机制*
